...
$(document).ready(function () {
    ...
    var table = $('#entityFee').DataTable({
        "serverSide": true,
        "proccessing": true,
        "ajax": {
            "url": "/EntityFee/GetEntityDetails",
            "type": "POST",
        ...
        columns: [
...

            {
                ...
                "render": function (data, type, full, meta) {
                    return '<a href="/EntityFee/_EditEntityFee?entityId=' + data.entityId + '" class="editEntityFee"><i class="fas fa-pencil-alt"></i></a>&nbsp;&nbsp;&nbsp;<a href="/Employee/_DeactivateEmployee?id=' + data.id + '" class="deactivateEmployee" title="Deactivate"><i class="fas fa-trash-alt" style="color:#dc3545"></i></a>';
                }
            }
        ],
        "language": {
...
$('#inspectorFee').on("click", ".editEntityFee", function (event) {
...
$('#sample-analysis-modal').on('show.bs.modal', function () {
    var EntityAuthorizationId = $("#EntityAuthorizationId").val();
    var table = $('#sample-analysis-list-table').DataTable({
        "serverSide": true,
        "proccessing": true,
        "ajax": {
            "url": "/EntityAuthorization/GetSampleAnalysis",
            "type": "POST",
            "data": { EntityAuthorizationId: EntityAuthorizationId }
        },
        columns: [
...
$('#sample-labs-modal').on('show.bs.modal', function () {
    ...
    var table = $('#sample-labs-list-table').DataTable({
        ...
        "proccessing": true,
        "ajax": {
            "url": "/EntityAuthorization/GetSampleLaboratory",
            "type": "POST",
            "data": { EntityAuthorizationId: EntityAuthorizationId }
        },
        columns: [
...
$(document).ready(function () {
    ...
    var table = $('#analysisreports').DataTable({
        ...
        "proccessing": true,
        "ajax": {
            "url": "/EntityAuthorization/GetSamplesDetails",
            "type": "POST",
        ...
        columns: [
...

            {
                ...
                "sortable": false,
                "render": function (data, type, full, meta) {
                    //return '<a class="view-analysis" href="/EntityAuthorization/_ViewSampleAnalysis?id=' + data.id + '&lotNumber=' + data.lotNumber + '"><button class="btn btn-info btn-xs"><i class="fas fa-vials" style="font-size:14px;color:white"></i> Analysis</button> </a>' + ' | '
                    //    + '<a class="view-labs" href="/EntityAuthorization/_ViewSampleLaboratory?id=' + data.id + '&lotNumber=' + data.lotNumber + '"><button class="btn btn-info btn-xs"><i class="glyphicon glyphicon-list-alt"></i> View Labs</button> </a>'
                    ...
                    if (data.sampleStatus === "Pending" && data.evaluationStatus === "Pending")
                        ...
                        action = '<a class="view-sample-evaluation" href="/EntityAuthorization/EntityAuthorization?id=' + data.id + '"><button class="btn btn-primary btn-xs"><i class="fas fa-cogs"></i> Analyse</button> </a>';
    ...
    table.on('draw.dt', function () {
        var PageInfo = $('#EntityAuthorization').DataTable().page.info();
        table.column(3, { page: 'current' }).nodes().each(function (cell, i) {
            cell.innerHTML = i + 1 + PageInfo.start;
        });
    });
    // Add event listener for opening and closing details
    $('#EntityAuthorization tbody').on('click', 'td.details-control', function () {
