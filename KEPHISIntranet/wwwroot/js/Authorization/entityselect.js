...
$("#EntityId").change(function () {
    ...
    getEntityDataTable(entityId); // load EntityIds by Id selected
    getEntityFeeDataTable(entityId);
});

function getEntityDataTable(entityId) {
    var table = $('#entitydetails').DataTable({
        "serverSide": true,
        "bFilter": false,
        "bLengthChange": false,
        "dom": "lfrti",
        ...
        columns: [
...

            {
                ...
                "render": function (data, type, full, meta) {
                    return '<a href="/EntityFee/AddEntityFee?entityId=' + data.id + '" class="add-fee"><button class="btn btn-success btn-xs"><i class="mdi mdi-plus-circle-outline"></i> Billing</button></a>';
                }
            }
        ],
        "language": {
...
function getEntityFeeDataTable(entityId) {
    var table = $('#vwpendingentityfee2').DataTable({
        "serverSide": true,
        "iDisplayLength": 100,
        dom: 'Bfrtip',
        buttons: [
            ...
            },
            {
                ...
                title: 'Authorization Entity Fees',
                exportOptions: {
                    columns: [0, 2, 4, 5, 6,7]
                },
        ...
        "footerCallback": function (row, data, start, end, display) {
            ...
            //$(api.column(6).footer()).html('Grand Total Pending Entity Fee(s)' + feeAmount).css({ "font-size": "1rem", "font-family": "Segoe UI Emoji", "font-weight": "bold", "text-decoration-line": "underline", "text-decoration-style": "double" });
        },
        "ajax": {
