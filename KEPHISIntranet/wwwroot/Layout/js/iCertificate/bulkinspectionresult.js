...
$(document).ready(function () {
    ...
    var table = $('#bulkResult').DataTable({
        "serverSide": true,
        "proccessing": true,
        "ajax": {
            "url": "/BulkInspectionResult/GetBulkInspectionResultList",
            "type": "POST"
        },
        columns: [
            {
            ...
            },
            {
                ...
                "render": function (data, type, full, meta) {
                    return '<a class="view-employee" href="/BulkInspectionResult/InspectionResultDetails?id=' + data.id + '"><i class="fas fa-edit"></i></a>  <a target="_blank" href="/AssignedInspectionResult/ConfirmPrintCertificate?id=' + data.id + '" class="printCert" data-toggle="tooltip" title="Print Cert"><i class="mdi mdi-printer" style="color:#595959;font-size:17px"></i></a>  <a href="/IssuedCertificates/ConfirmRejectCertificate?id=' + data.id + '" class="rejectCert" title="Reject Cert"><i class="fas fa-trash-alt" style="color:#dc3545"></i></a>';
                }
            }
        ]
    });
    ...
    table.on('draw.dt', function () {
        var PageInfo = $('#bulkResult').DataTable().page.info();
        table.column(0,{ page: 'current' }).nodes().each(function (cell, i) {
            cell.innerHTML = i + 1 + PageInfo.start;
        });
    });
    $('#bulkResult').on("click", ".printCert", function (event) {
    ...
    $('#assignmentResult').on("click", ".rejectCert", function (event) {
        event.preventDefault();
        var url = $(this).attr("href");
        $.get(url, function (data) {
